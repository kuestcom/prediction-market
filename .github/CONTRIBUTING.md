...
- Run `pnpm lint`, `pnpm format:check`, `pnpm test`, and `pnpm build` before submitting.
...
7. Run `pnpm lint`, `pnpm format:check`, `pnpm test`, and `pnpm build`
...
- `pnpm lint` checks the repository with Oxlint.
...
- VS Code-compatible editors use the recommended Oxc extension and save actions from `.vscode`.
