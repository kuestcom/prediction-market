...
Submit batched contract calls through the {ENV.NEXT_PUBLIC_SITE_NAME} L2 relay. This router shares the exact authentication headers used by [`POST /order`](/api-reference/trade/post-a-new-order) and executes transactions from the managed proxy wallet tied to your API key.
