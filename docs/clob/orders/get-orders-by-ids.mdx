...
Request multiple orders (between 1 and 500 ULIDs) in a single call. Each entry mirrors [`GET /data/order/{id}`](/api-reference/trade/get-single-order-by-id), so you can refresh many orders without issuing repeated requests.
