...
_openapi:
  method: GET
---
<code>{process.env.DATA_URL}/trades</code> surfaces every fill captured by the {process.env.NEXT_PUBLIC_SITE_NAME} orderbook subgraph. You can query by wallet or by a batch of markets.
