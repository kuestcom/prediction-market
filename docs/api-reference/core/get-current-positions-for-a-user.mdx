...
_openapi:
  method: GET
---
<code>GET {process.env.DATA_URL}/positions</code> returns every open claim held by a wallet, enriched with market metadata (titles, slugs, icons, and event slugs). The `user` query parameter is mandatory and must be an EVM address.
