...
<code>{process.env.DATA_URL}/activity</code> returns a ledger-style list that interleaves trades, splits, merges, redemptions, conversions, and rewards. Each row is enriched with market metadata and profile info so your UI can display avatars and event titles without extra queries.
