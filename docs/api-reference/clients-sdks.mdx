...
| Split collateral into outcome token inventory | Builder Relayer client or Rust CLOB client |
| Merge complete token sets back to collateral | Builder Relayer client or Rust CLOB client |
| Redeem resolved winning positions | Builder Relayer client or Rust CLOB client |
...
For a binary market, splitting `100` collateral units creates `100` YES tokens and `100` NO tokens. You can run the split through the relayer as shown above, or use the Rust CLOB client when your service wallet is funded for direct onchain transactions.
