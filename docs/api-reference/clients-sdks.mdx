...
<Cards>
  ...
  >
    ...
    <p>Packages: <code>@kuestcom/clob-client</code>, <code>kuest-py-clob-client</code>, and <code>kuest-client-sdk</code>.</p>
  </Card>
  ...
  >
    <p>Available in TypeScript, Python, and Rust.</p>
    <p>Packages: <code>@kuestcom/builder-relayer-client</code>, <code>kuest-py-builder-relayer-client</code>, and <code>kuest-builder-relayer-client</code>.</p>
  </Card>
</Cards>

<Callout type="info" title="Personalized downloads">
  Download your SDK bundle from <a href="/settings/sdks">Settings / SDKs</a>. Choose CLOB when you need market data and order placement, or Builder Relayer when you need deposit wallet deployment and signed wallet actions.
</Callout>
...
| Split collateral into outcome token inventory | Builder Relayer client or Rust CTF client |
| Merge complete token sets back to collateral | Builder Relayer client or Rust CTF client |
| Redeem resolved winning positions | Builder Relayer client or Rust CTF client |
...
For a binary market, splitting `100` collateral units creates `100` YES tokens and `100` NO tokens. You can run the split through the relayer as shown above, or use the Rust CTF client from `kuest-client-sdk` when your service wallet is funded for direct onchain transactions.
...
# async fn split_inventory(condition_id: B256) -> anyhow::Result<()> {
