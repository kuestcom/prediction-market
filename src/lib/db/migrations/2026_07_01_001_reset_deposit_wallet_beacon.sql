CREATE TEMP TABLE deposit_wallet_beacon_reset_users ON COMMIT DROP AS
SELECT id
FROM users
...
SET
  ...
  settings = CASE
    ...
    ELSE '{}'::jsonb
  END
WHERE id IN (SELECT id FROM deposit_wallet_beacon_reset_users);

DELETE FROM sessions
WHERE user_id IN (SELECT id FROM deposit_wallet_beacon_reset_users);
